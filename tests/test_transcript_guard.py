...
# Wake-only against the default keyword ("spire", used when no wake_phrase
# is configured) -- an echo of the wake phrase, or a near-miss of it.
_WAKE_ONLY = ("Space Spire", "Stay Spire", "Hey, Spire.", "spire")
...
_COMMANDS = ("turn off the spire lamp", "what's the weather in paris", "turn on the lights", "stop")
...
@pytest.mark.parametrize("text", ("Space Spire", "Hey Spire, uh"))
def test_wake_only_with_configured_wake_phrase_is_no_command(text: str) -> None:
    assert is_no_command(text, "hey spire") is True
...
@pytest.mark.parametrize("wake_phrase", (None, "hey spire"))
...
def test_keyword_comes_from_the_configured_wake_phrase() -> None:
    """The last word of `wake_phrase`, not the default "spire", is the
    keyword this rule matches against."""
    assert is_no_command("computer", "okay computer") is True
    assert is_no_command("spire", "okay computer") is False
...
def test_blank_or_missing_wake_phrase_falls_back_to_the_default_keyword(wake_phrase: str | None) -> None:
    assert is_no_command("spire", wake_phrase) is True
