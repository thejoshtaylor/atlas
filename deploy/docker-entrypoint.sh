#!/usr/bin/env bash
# Docker Compose's own D-16 mechanism: SPIRE_SECRET_KEY is generated once,
...
# (src/spire_voice/auth/tokens.py). A key that changes across a restart
...
DATA_DIR="${SPIRE_DATA_DIR:-/data}"
SECRET_FILE="${DATA_DIR}/secret_key"

if [ -z "${SPIRE_SECRET_KEY:-}" ]; then
  if [ ! -f "$SECRET_FILE" ]; then
    # The exact generation call src/spire_voice/config.py::read_secret_key
  ...
  SPIRE_SECRET_KEY="$(cat "$SECRET_FILE")"
  export SPIRE_SECRET_KEY
fi
