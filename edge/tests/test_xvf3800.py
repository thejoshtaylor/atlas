"""Proves atlas_edge.xvf3800's vendor-control request shape, retry-while-
busy behavior, and payload decoding against a fake device object -- the
same proof edge/tests/test_xvf3800_control.py already gives the spike
module, now against the runtime copy plan 10-09 moved it into. No real
USB hardware anywhere in this file."""
...
from atlas_edge.xvf3800 import (
    PARAMETERS,
    XvfControlError,
    XvfNotFound,
...
def test_lazy_usb_import() -> None:
    """Importing atlas_edge.xvf3800 must never import usb.core at module
    scope -- confirmed by asserting the module has no such attribute
    reachable without calling find_device()."""
