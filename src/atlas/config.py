"""Load, expand, and validate atlas's configuration.
...
2. Every config-shaped section builds itself through a `from_config`
   ...
   `atlas_mcp.safety.Policy`'s own doctrine: raised, not returned, so a
...
class SecurityConfig:
    ...
    secret_key_env: str = "ATLAS_SECRET_KEY"
    access_token_ttl_s: int = 900
    refresh_token_ttl_s: int = 1209600
    cookie_name: str = "atlas_session"
...
class OpenWakeWordConfig:
    ...
    model_path: str = "/models/hey_atlas.onnx"
...
class WakeConfig:
    ...
    `openwakeword`: no "hey atlas" model exists for openWakeWord yet, and
...
class GateConfig:
    ...
    denylist in `mcp/atlas_mcp/safety.py` is what protects it, not identity.
...
class MacroActionConfig:
    ...
    (`mcp/atlas_mcp/safety.py`) both check it at fire time, and duplicating
...
def _check_macros_do_not_collide(macros: "Sequence[MacroConfig]") -> None:
    ...
    `Sequence[atlas.db.repository.Macro]` -- that class's own
