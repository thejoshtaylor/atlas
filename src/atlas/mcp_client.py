"""The stdio MCP client wrapper: owns `atlas_mcp.ha`'s subprocess for the
...
`-m atlas_mcp.ha` resolve. Building it literally, not by copying and
...
_DEFAULT_CHILD_MODULE = "atlas_mcp.ha"
...
class McpToolHost:
    ...
    def __init__(self) -> None:
        ...
        # `HA_TOKEN`, no `ATLAS_SAFETY`, nothing merged in from anywhere
    ...
    async def call_tool(self, name: str, arguments: dict[str, Any]) -> Any:
        ...
        tool-level failure. A `Denied` raised inside `atlas_mcp.ha` becomes,
        on the wire, `CallToolResult(is_error=True, content=[TextContent(text=
        str(exc))])` -- and `str(exc)` on a `Denied` is exactly `reason` (see
        `atlas_mcp.safety.Denied.__init__`). The turn controller reads
