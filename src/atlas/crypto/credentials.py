...
`atlas.auth.tokens` already reserved for this purpose -- that
...
operator-supplied `ATLAS_SECRET_KEY` yields two independent derived keys
...
already imports `atlas.routes`, which imports
...
from atlas.auth.tokens import _CREDENTIAL_ENCRYPTION_INFO, _derive_key
from atlas.config import Config, SecurityConfig, read_secret_key
from atlas.db.repository import CredentialRepository
