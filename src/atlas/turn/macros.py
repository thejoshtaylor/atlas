...
passes `allow_call` (`mcp/atlas_mcp/safety.py`) at fire time, exactly like a
model-issued call: a macro is a shortcut past the model, never past the
boundary.

Import direction, load-bearing: `atlas.config` imports `normalize` from
this module, so this module must never import `atlas.config` at
...
if TYPE_CHECKING:
    # Annotation-only: `atlas.config` imports `normalize` from this
...
async def fire_macro(
    ...
    *,
    ...
    model-issued tool call uses -- so `allow_call` (`mcp/atlas_mcp/safety.py`)
