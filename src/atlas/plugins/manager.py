...
from atlas.db.repository import Plugin, PluginConfigValue, PluginRepository
...
# where the child imports `atlas_mcp` -- including `atlas_mcp.safety` --
# from, and `ATLAS_SAFETY` carries the house policy the enforcing child
    ...
    *,
    ...
    for value in config_values:
        if value.key in RESERVED_ENV_KEYS:
            ...
            # decided where the child imports `atlas_mcp.safety` from --
            # for the policy-enforcing row, that is the one in-child
            # boundary this system has. `ATLAS_SAFETY` was already safe by
