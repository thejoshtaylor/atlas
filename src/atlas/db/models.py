...
class SafetyPolicyRow(Base):
    ...
    `mcp/atlas_mcp/safety.py`'s `Policy.mode` is a single `Literal`: a state
...
class MacroRow(Base):
    ...
    `phrase`/`reply` are exactly `atlas.config.MacroConfig.phrase`/
...
class MacroActionRow(Base):
    ...
    `arguments` shape `atlas.config.MacroActionConfig` already
...
class WorkflowStepRow(Base):
    ...
    (`atlas.config.WorkflowConfig.max_attempts`); `result_detail`
...
class ProviderCredentialRow(Base):
    ...
    `atlas.crypto.credentials.CredentialSlot` names -- a route
    ...
    `atlas.crypto.credentials.encrypt_credential` returns; nothing
...
class PluginRow(Base):
    ...
    `PluginManager` (`atlas.plugins.manager`) ever spawns a child
    ...
    mirroring `atlas.config.McpServerConfig.args`'s own refusal of a
...
class PluginConfigValueRow(Base):
    ...
    exactly what `atlas.crypto.credentials.encrypt_credential`
...
class ProviderSelectionRow(Base):
    ...
    `slot` is one of the three names `atlas.providers.registry`
