...
from atlas.turn.macros import normalize as _normalize_macro_key
...
class PostgresPolicyRepository:
    ...
    Structurally satisfies `atlas.db.repository.PolicyRepository`
...
# do). Spelled out as the ASCII bytes of "atlas-admin", stuffed into a
# 63-bit int -- readable in a `pg_locks` dump, not that it needs to be.
_CREATE_FIRST_USER_LOCK_KEY = int.from_bytes(b"atlasadm", "big") & 0x7FFFFFFFFFFFFFFF


class PostgresAccountRepository:
    """`AccountRepository`, implemented against a real Postgres.

    Structurally satisfies `atlas.db.repository.AccountRepository` (a
...
class PostgresSetupRepository:
    ...
    Structurally satisfies `atlas.db.repository.SetupRepository` (a
    `typing.Protocol`) -- there is no base class to inherit from, matching
...
class PostgresSettingsRepository:
    ...
    Structurally satisfies `atlas.db.repository.SettingsRepository`
    (a `typing.Protocol`) -- there is no base class to inherit from,
...
class PostgresProviderSelectionRepository:
    ...
    Structurally satisfies `atlas.db.repository.ProviderSelectionRepository`
...
class PostgresWakeEventRepository:
    ...
    Structurally satisfies `atlas.db.repository.WakeEventRepository`
...
class PostgresCredentialRepository:
    ...
    Structurally satisfies `atlas.db.repository.CredentialRepository`
    (a `typing.Protocol`) -- there is no base class to inherit from,
...
class _CandidateMacro:
    ...
    imports `atlas.db.repository` (transitively, this module) --
...
# fix. Spelled out as the ASCII bytes of "atlasmac1", stuffed into a
...
_MACRO_WRITE_LOCK_KEY = int.from_bytes(b"atlasmac1", "big") & 0x7FFFFFFFFFFFFFFF


class PostgresMacroRepository:
    """`MacroRepository`, implemented against a real Postgres.

    Structurally satisfies `atlas.db.repository.MacroRepository` (a
    `typing.Protocol`) -- there is no base class to inherit from, matching
...
class PostgresPluginRepository:
    ...
    Structurally satisfies `atlas.db.repository.PluginRepository` (a
    `typing.Protocol`) -- there is no base class to inherit from, matching
    every other `Postgres*Repository` class in this module.
...
class PostgresWorkflowRepository:
    ...
    Structurally satisfies `atlas.db.repository.WorkflowRepository`
    (a `typing.Protocol`) -- there is no base class to inherit from,
    matching every other `Postgres*Repository` class in this module.
