...
Follows `atlas.providers.base`'s `Protocol`-typed shape exactly --
...
class PolicyRule:
    ...
    than the ORM row itself -- a caller outside `src/atlas/db/` has
...
class PolicyRepository(Protocol):
    ...
    `mcp/atlas_mcp/safety.py` enforces, `list_rules` is the raw rows the
    ...
    async def add_rule(
        ...
        against the shapes `mcp/atlas_mcp/safety.py` enforces before
    ...
    async def set_mode(self, mode: str, *, updated_by_user_id: int | None) -> None:
        ...
        validating `mode` against `atlas_mcp.safety.Mode`'s two literal
...
class MacroAction:
    ...
    `arguments` shape `atlas.config.MacroActionConfig` carries, plus
...
class Macro:
    ...
    Duck-type compatible with `atlas.config.MacroConfig` on purpose:
    `.phrase`, `.reply`, `.actions` (each exposing `.tool`/`.arguments`),
    and `.normalized_keys` below are exactly the shape `turn/macros.py`'s
    `match()`/`fire_macro()` and `atlas.config._check_macros_do_not_
    ...
    def normalized_keys(self) -> frozenset[str]:
        ...
        (`atlas.config`) performs, over this row's own phrase and
        aliases rather than a parsed config block -- duplicated rather
        than imported, since importing `atlas.config` here would
...
class MacroRepository(Protocol):
    ...
    (`atlas.config._check_macros_do_not_collide`, callable directly
...
class User:
    ...
    outside `src/atlas/db/` has no reason to hold a SQLAlchemy-mapped
...
class Credential:
    ...
    `atlas.crypto.credentials.decrypt_credential` (see that module's
...
class CredentialRepository(Protocol):
    ...
    `atlas.crypto.credentials`, and only at the one call site that
    ...
    async def list_credentials(self) -> Sequence[Credential]:
        ...
        `atlas.crypto.credentials.CredentialSlot`, not here; a
...
class PluginConfigValue:
    ...
    (`atlas.plugins.host`) is the one place `ciphertext` is ever
    decrypted.
    """
...
class Plugin:
    ...
    rather than ORM rows: a caller outside `src/atlas/db/` has no
...
class PluginRepository(Protocol):
    ...
    (`atlas.plugins.manager`) actually uses (D-01, D-04), plus the
    ...
    async def get_config_values(self, plugin_id: int) -> Sequence[PluginConfigValue]:
        ...
        is built key by key from this list (`atlas.plugins.host`),
