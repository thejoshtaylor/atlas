...
(`mcp/atlas_mcp/safety.py`) and the exact same kind of live-catalog read
...
Every refusal the file-parsed `atlas.config.MacroConfig`/
...
(`atlas.config._check_macros_do_not_collide`) -- reused directly
against the repository's own `Macro` objects (duck-type compatible, see
`atlas.db.repository.Macro`'s own docstring), never restated.
...
from atlas.db.repository import Macro, MacroRepository
from atlas.providers.tts_cache import precache_all, precache_other_sinks
from atlas.routes.conflict import (
    ConflictAnnotation,
    annotate_conflict,
...
def _zero_actions_error(phrase: str) -> HTTPException:
    """The exact reason `atlas.config.MacroConfig.from_config`
...
class _CandidateMacro:
    ...
    exactly like `atlas.db.repository.Macro`'s own duck-typed
...
async def _check_no_collision(
    ...
    *,
    ...
    try:
        ...
        # module's own import time (`from atlas.config import
        # ConfigError`): `tests/test_config.py`'s own
        # `test_config_and_turn_macros_import_in_either_order` reloads
        # `atlas.config` in place, which replaces `ConfigError` with
        ...
        # currently in `sys.modules['atlas.config']`, so this stays
        # correct across a reload the same way `atlas.config`'s own
