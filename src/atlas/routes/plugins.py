...
from atlas.db.repository import Plugin, PluginAlreadyExistsError, PluginConfigValue, PluginRepository
...
from atlas.plugins.host import module_for_stdio_args, validate_remote_url
from atlas.plugins.manager import REMOTE_AUTH_KEY, RESERVED_ENV_KEYS
...
def _reserved_config_key_error(keys: Sequence[str]) -> HTTPException:
    """WR-08 (code review): `PYTHONPATH` and `ATLAS_SAFETY` are written by
    this process, not by a plugin's configuration -- the first decides
    where the child imports `atlas_mcp.safety` from, the second carries
