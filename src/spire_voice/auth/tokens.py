...
independent values derived from the one `SPIRE_SECRET_KEY` the operator
...
class InvalidAccessToken(Exception):
    """Raised instead of returned, matching `spire_mcp.safety.Denied`'s and
    `spire_voice.config.ConfigError`'s own doctrine: an expired, malformed,
