...
end-of-speech, and an operator who pauses for even a beat after "hey spire"
gets exactly that pause read as the end of the utterance -- the wake phrase
itself, or a mangled tail of it ("fire" for "hey spire"), becomes the whole
...
("hey spire", "spire" for the default two-word phrase), never against the
phrase as a whole alone -- a short reply that only echoes the tail ("fire")
...
so a real command that happens to share a fuzzy-matchable prefix ("fire the
lights" against "spire") is never misread as wake-only by length alone.
Both checks run on lowercased text -- the word-count check additionally
strips punctuation and collapses whitespace first (`"hey, spire"` splits
into two words, not `["hey,", "spire"]` read as if the comma mattered to
...
transcript ("hey, spire") should still read as extremely close to the
...
separates the wake-only samples this task's own evidence produced ("ace
fire" 0.59, "fire" 0.67, "hey, spire" 0.95, "a spire" 0.83) from the
genuine-command samples that must never trip this check ("stop" 0.44, "turn
on the lights" 0.30, "fire the lights" 0.30) -- provisional until a real
