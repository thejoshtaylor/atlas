"""Per-turn stage timing: seven timestamps and their derived durations,
nothing else.
...
The seven stages, in the order a turn reaches them: the turn starting, the
...
completing, and the first audio chunk leaving for the browser. A turn closed
early by either VOICE-08 guard reaches only a prefix of these -- an unset
(`None`) stage is what "never reached" looks like, distinguishable from a
reached stage that happened to take zero time.
"""
