...
`atlas_mcp.safety.Denied.__init__`). That is the mechanism, not an accident:
...
def _load_policy() -> Policy:
    ...
    """
    raw = os.environ.get("ATLAS_SAFETY")
    if raw is None:
        return Policy.from_config(None)
    try:
        block = json.loads(raw)
    except json.JSONDecodeError as exc:
        raise SystemExit(f"ATLAS_SAFETY is not valid JSON: {exc}") from exc
    if block is not None and not isinstance(block, dict):
        raise SystemExit(f"ATLAS_SAFETY must be a mapping, got {type(block).__name__}")
